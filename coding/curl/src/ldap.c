...
*/
    ...
    server = ldap_init((PWCHAR)conn->host.name, (int)conn->port);
    if(server == NULL) {
      failf(data, "LDAP local: Cannot connect to %s:%ld",
              conn->host.name, conn->port);
      status = CURLE_COULDNT_CONNECT;
  ...
  rc = ldap_simple_bind_s(server,
                          conn->bits.user_passwd ? (PWCHAR)conn->user : NULL,
                          conn->bits.user_passwd ? (PWCHAR)conn->passwd : NULL);
  if(!ldap_ssl && rc != 0) {
  ...
  rc = ldap_search_s(server, (PWCHAR)ludp->lud_dn, ludp->lud_scope,
                     (PWCHAR)ludp->lud_filter, (PWCHAR*)ludp->lud_attrs, 0, &result);
  ...
  for(num = 0, entryIterator = ldap_first_entry(server, result);
    ...
    char  *dn = (char*)ldap_get_dn(server, entryIterator);
    int i;
    ...
    for(attribute = (char*)ldap_first_attribute(server, entryIterator, &ber);
        attribute;
        attribute = (char*)ldap_next_attribute(server, entryIterator, ber)) {
      BerValue **vals = ldap_get_values_len(server, entryIterator, (PWCHAR)attribute);
        ...
        for(i = 0; (vals[i] != NULL); i++) {
          ...
          if((strlen(attribute) > 7) &&
            ...
            if(error) {
              ...
              ldap_memfree((PWCHAR)attribute);
              ldap_memfree((PWCHAR)dn);
              if(ber)
                ber_free(ber, 0);
              status = error;
              goto quit;
