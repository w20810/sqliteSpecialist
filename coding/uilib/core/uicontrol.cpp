...
namespace UiLib
{
	...
	int CControlUI::GetCtrlIndex() const
	{
		return m_nCtrlIndex;
	}
	...
	void CControlUI::SetCtrlIndex(int nIndex)
	{
		m_nCtrlIndex = nIndex;
	}
	...
	void CControlUI::SetIcon(HICON icon, int width, int height)
	{
		if (m_hIcon == icon) return;
	...
	void CControlUI::SetIconAlign(CDuiString align)
	{
		if( _tcsstr(align, _T("left")) != NULL ) {
			m_hIconAlign = DT_LEFT;
		}
		if( _tcsstr(align, _T("center")) != NULL ) {
			m_hIconAlign = DT_CENTER;
		}
		if( _tcsstr(align, _T("right")) != NULL ) {
			m_hIconAlign = DT_RIGHT;
		}
	}
	...
	void CControlUI::SetStyleName( LPCTSTR pStrStyleName,CPaintManagerUI* pm /*= NULL*/ )
	{
		...
		if(!pStyleMap)
		{
			...
			if (!pPaintManager)
				::MessageBoxW(0, L"PaintManager为空，请检查style.xml文件是否合法", L"警告", MB_OK);

			ASSERT(pPaintManager);
			if (GetManager()->GetParentResource() != NULL)
				pStyleMap = GetManager()->GetParentResource()->GetControlStyles(pStrStyleName);
	...
	void CControlUI::SetAttribute(LPCTSTR pstrName, LPCTSTR pstrValue)
	{
		...
		else if( _tcscmp(pstrName, _T("ctrlindex")) == 0 ) SetCtrlIndex(_ttoi(pstrValue));
	...
	void CControlUI::PaintIcon(HDC hDC)
	{
		if (m_hIcon == NULL) return;

		int left = m_rcItem.left + (m_rcItem.right -m_rcItem.left - m_iconWidth) / 2;
		int top = m_rcItem.top + (m_rcItem.bottom -m_rcItem.top - m_iconHeight) / 2;

		if (m_hIconAlign == DT_LEFT)
		{
			left = m_rcItem.left;
		}
		else if (m_hIconAlign == DT_RIGHT)
		{
			left = m_rcItem.right - m_iconWidth;
		}

		DrawIconEx(hDC, left, top, m_hIcon, m_iconWidth, m_iconHeight, 0, NULL, DI_NORMAL);
	}
