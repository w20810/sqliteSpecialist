...
#define IDB_ICONS50 203
	...
	CDuiString CPaintManagerUI::GetInstanceApp()
	{
		TCHAR tszModule[MAX_PATH + 1] = { 0 };
		::GetModuleFileName(m_hInstance, tszModule, MAX_PATH);
		return tszModule;
	...
	bool CPaintManagerUI::CreateDir(LPCTSTR path, bool bRelative)
	{
		CDuiString createPath = path;
		if (bRelative)
		{
			CDuiString absolute = GetInstancePath();
			createPath = absolute + L"\\" + createPath;
		}

		if (!PathIsDirectory(createPath))
		{
	...
	void CPaintManagerUI::SetHSL(bool bUseHSL, short H, short S, short L)
	{
		...
		m_H = CLAMP(H, (short)0, (short)360);
		m_S = CLAMP(S, (short)0, (short)200);
		m_L = CLAMP(L, (short)0, (short)200);
	...
	void CPaintManagerUI::ReloadStyle(CDuiString sStyleXml /* = _T */)
	{
		for( int i = 0; i < m_aPreMessages.GetSize(); i++ ) {
			CPaintManagerUI* pManager = static_cast<CPaintManagerUI*>(m_aPreMessages[i]);
			CDuiString sStyleName = L"style";
			CDialogBuilder builder;
			builder.CreateStyle(sStyleName, sStyleXml, pManager);
			pManager->SetCurStyles(sStyleName);
		}
	}
	...
	int CPaintManagerUI::GetStrFixedWidth(CDuiString duiStr)
	{
		RECT rcText = {0,0,9999,9999};
		CRenderEngine::DrawText(GetPaintDC(), this, rcText, duiStr, 0, 1, DT_SINGLELINE|DT_CALCRECT);
	   ...
	   case WM_PAINT:
		   {
			   ...
			   if(m_bAlphaBackground)
			   {
				   ...
				   if (!m_bIsRestore)
				   {
					   IntersectRect(&m_rcInvalidate, &m_rcInvalidate, &rcClient);
		...
		case WM_MOUSEHOVER:
			{
				...
				::SendMessage(m_hwndTooltip, TTM_SETMAXTIPWIDTH, 0, 2400);
			}
			return true;
		case WM_MOUSELEAVE:
			{
...
// 			break;
		...
		case WM_MOUSEWHEEL:
			{
				...
				if (_tcsicmp(pControl->GetClass(),DUI_CTR_WKEWEBKIT)==0)
					event.wParam = wParam;
				else
	...
	bool CPaintManagerUI::AttachDialog(CControlUI* pControl)
	{
		...
		m_bUpdateNeeded = true;
		m_bFirstLayout = true;
		m_bFocusNeeded = false; // 改为false，否则默认搜索框获得焦点
	...
	void CPaintManagerUI::SetFocus(CControlUI* pControl)
	{
		...
		if( hFocusWnd != m_hWndPaint && pControl != NULL) 
			::SetFocus(m_hWndPaint);
		// Already has focus? 当点击窗体外之后再次点击richedit就会return，bug
	...
	void CPaintManagerUI::EventAllControl( TEventUI& event,CControlUI* pControl /*= NULL*/ )
	{
		...
		// UIList消息分发也要遍历header
		if (pContainer && _tcscmp(pControl->GetClass(), _T("ListUI")) == 0)
		{
			CListUI* pList = static_cast<CListUI*>(pControl);
			if (pList && pList->GetHeader())
				EventAllControl(event, pList->GetHeader());
		}
	...
	HFONT CPaintManagerUI::GetFont(int index)
	{
		...
		{
			if (m_pParentResourcePM != 0 && index < (int)m_pParentResourcePM->GetCustomFontCount()) // 修复combo控件绘制item字体的问题，CComboWnd没有字体，需要拿父窗口的字体
			{
	...
	CControlUI* CPaintManagerUI::FindSubControlByIndex(CControlUI* pParent, int nIndex) const
	{
		if( pParent == NULL ) pParent = GetRoot();
		ASSERT(pParent);
		return pParent->FindControl(__FindControlFromIndex, &nIndex, UIFIND_ALL);
	}
	...
	CControlUI* CALLBACK CPaintManagerUI::__FindControlFromIndex(CControlUI* pThis, LPVOID pData)
	{
		int* nIndexValue = static_cast<int*>(pData);
		const int& nIndex = pThis->GetCtrlIndex();
		if( nIndex == -1) return NULL;
		return (nIndex == *nIndexValue) ? pThis : NULL;
	}
