...
namespace UiLib
{
	...
	void CDialogBuilder::CreateStyle(LPCTSTR styleName, LPCTSTR styleXml, CPaintManagerUI* pManager)
	{
		LPCTSTR pstrName = NULL;
		LPCTSTR pstrValue = NULL;
		CMarkup nStyleFile;
		if(nStyleFile.LoadFromFile(styleXml))
		{
			for(CMarkupNode nStyleNode = nStyleFile.GetRoot().GetChild(); nStyleNode.IsValid(); nStyleNode = nStyleNode.GetSibling())
			{
				LPCTSTR pstrClass = nStyleNode.GetName();
				...
				for( int nIndex = 0; nIndex < nStyleAttributes; nIndex++ ) 
				{
					...
					if( _tcscmp(pstrName, _T("stylename")) == 0 )
						continue;
					pManager->SetControlStyle(nControlStyleName.GetData(), pstrName, pstrValue, styleName);
				}
			}
		}
	}
		...
		for( CMarkupNode node = pRoot->GetChild() ; node.IsValid(); node = node.GetSibling() ) {
			...
			//树控件XML解析
				...
				case 8:
					...
					else if( _tcscmp(pstrClass, DUI_CTR_OPTIONEX) == 0 )          pControl = new COptionExUI;
					else if( _tcscmp(pstrClass, DUI_CTR_BUTTONEX) == 0 )          pControl = new CButtonExUI;
					break;
				case 9:
				...
				case 11:
					...
					else if (_tcscmp(pstrClass, DUI_CTR_DYNAMICLIST) == 0)		  pControl = new CDynamicListUI;
					break;
				case 14:
				...
				case 16:
					...
					else if (_tcscmp(pstrClass, DUI_CTR_EDITDISABLERMENU) == 0 )   pControl = new CEditDisableRMenuUI;
					break;
				case 18:
				...
				case 20:
					...
					else if( _tcscmp(pstrClass, DUI_CTR_HORIZONTALLAYOUTFUNC) == 0)  pControl = new CHorizontalLayoutFuncUI;
					break;
				}
