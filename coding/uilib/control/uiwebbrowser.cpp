...
CIEWndMap UiLib::CWebBrowserUI::m_browserWndMap;
...
{
	if (pDisp == m_pWebBrowser2 && m_pWebBrowserEventHandler && !m_bNavigateError)
	{
		m_pWebBrowserEventHandler->DocumentComplete(pDisp, url);
	}
}

void UiLib::CWebBrowserUI::ParseServerWnd()
{
	...
	//1. --Shell Embedding
	if (FAILED(m_pWebBrowser2->QueryInterface(IID_IServiceProvider, (void**)&pServiceProvider)))
		return;

	IOleWindow* pWindow = NULL;
	if (FAILED(pServiceProvider->QueryService(SID_SShellBrowser, IID_IOleWindow, (void**)&pWindow)))
	{
	...
	m_hServerWnd = hwndBrowser;
	pServiceProvider->Release();
	pWindow->Release();
}

void UiLib::CWebBrowserUI::RegIEProc()
{
	m_prevProc = (WNDPROC)GetWindowLongPtr(m_hServerWnd, GWLP_WNDPROC);
	SetWindowLongPtr(m_hServerWnd, GWLP_WNDPROC, (LONG)UiLib::CWebBrowserUI::IEProc);
	m_browserWndMap[m_hServerWnd] = this;
}

void UiLib::CWebBrowserUI::UnRegIEProc()
{
	SetWindowLongPtr(m_hServerWnd, GWLP_WNDPROC, (LONG)m_prevProc);

	CIEWndMap::iterator it = m_browserWndMap.find(m_hServerWnd);

	if (it != m_browserWndMap.end())
		m_browserWndMap.erase(it);
}

LRESULT UiLib::CWebBrowserUI::IEProcInner(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	if (message == WM_KILLFOCUS)
		m_pWebBrowserEventHandler->OnIEWndKillFocus(message, wParam, lParam);
	else if (message == WM_LBUTTONDOWN)
		m_pWebBrowserEventHandler->OnIEWndLButtonDown(message, wParam, lParam);
	else if (message == WM_LBUTTONUP)
		m_pWebBrowserEventHandler->OnIEWndLButtonUp(message, wParam, lParam);
	else if (message == WM_MOUSEMOVE)
		m_pWebBrowserEventHandler->OnIEWndMouseMove(message, wParam, lParam);
	else if (message == WM_KEYDOWN)
		m_pWebBrowserEventHandler->OnIEWndKeyDown(message, wParam, lParam);
	else if (message == WM_ERASEBKGND)
 		return TRUE;

	return CallWindowProc((WNDPROC)m_prevProc, hWnd, message, wParam, lParam);
}
...
{
	if(pMsg->message == WM_MOUSEWHEEL && (pMsg->wParam & MK_CONTROL))
		return S_OK;
...
void UiLib::CWebBrowserUI::NavigateUrl( LPCTSTR lpszUrl )
{
	...
	{
		BSTR bstr = SysAllocString(lpszUrl);
		m_pWebBrowser2->Navigate(bstr,NULL,NULL,NULL,NULL);
		SysFreeString(bstr);
	}
}
...
LRESULT CALLBACK UiLib::CWebBrowserUI::IEProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	CIEWndMap::iterator it = m_browserWndMap.find(hWnd);
	if (it == m_browserWndMap.end() || it->second == NULL)
		return 0;

	return it->second->IEProcInner(hWnd, message, wParam, lParam);
}

HWND UiLib::CWebBrowserUI::GetServerWnd()
{
	if (m_hServerWnd == NULL)
	{
		ParseServerWnd();
		RegIEProc();
	}
