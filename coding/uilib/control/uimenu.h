...
#define WM_MENUCLICK WM_APP + 121  //用来接收按钮单击的消息
...
public:
	...
	virtual UINT GetReceiversCount() const
	{
		return pReceivers_->size();
	}
...
public:
	...
	void SetHoverIcon(LPCTSTR strIcon);
	void SetIconSize(LONG cx, LONG cy);
	void SetExplandIconSize(LONG cx, LONG cy);
	void DrawItemIcon(HDC hDC, const RECT& rcItem);
	void DrawItemHoverIcon(HDC hDC, const RECT& rcItem);
...
protected:
	...
	CDuiString	m_strHoverIcon;
	bool		m_bCheckItem;

	UINT		m_uMenuElementState;

	bool		m_bShowExplandIcon;
	SIZE		m_szExplandIconSize;
};

UILIB_API ObserverImpl& GetObserverImplInstance();
