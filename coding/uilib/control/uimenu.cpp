...
void CMenuWnd::Init(CMenuElementUI* pOwner, STRINGorID xml, POINT point,
					...
					DWORD dwAlignment/* = eMenuAlignment_Left | eMenuAlignment_Top*/, bool bDynamic/* = false*/)
{
...
void CMenuWnd::OnFinalMessage(HWND hWnd)
{
	...
	if (m_stepParam.type == 3)
	{
		if (m_pOwner != NULL)
			::PostMessage(GetParent(hWnd), WM_KILLFOCUS, m_stepParam.wParam, m_stepParam.lParam);
		else
			m_pm.SendNotify(GetMenuUI(), DUI_MSGTYPE_CTLWND_KILLFOCUS, m_stepParam.wParam, m_stepParam.lParam);
	}
...
void CMenuWnd::ResizeSubMenu()
{
	...
	int cxFixed = 0;
	int cyFixed = m_pLayout->GetInset().top + m_pLayout->GetInset().bottom;
...
LRESULT CMenuWnd::OnKillFocus(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& bHandled)
{
	...
	{
		m_stepParam.type = 3;
		m_stepParam.hWnd = m_hWnd;
		m_stepParam.wParam = wParam;
		m_stepParam.lParam = lParam;
		Close();
...
m_uMenuElementState(0)
{
	...
	m_szExplandIconSize.cy = ITEM_DEFAULT_EXPLAND_ICON_SIZE;
	m_szExplandIconSize.cx = ITEM_DEFAULT_EXPLAND_ICON_SIZE;
...
void CMenuElementUI::DoPaint(HDC hDC, const RECT& rcPaint)
{
	...
	else
	{
		...
		if (((m_uMenuElementState & UISTATE_HOT) != 0 || IsSelected()) && m_strHoverIcon != _T(""))// 悬停
			DrawItemHoverIcon(hDC, m_rcItem);
		else
...
void CMenuElementUI::DrawItemHoverIcon(HDC hDC, const RECT& rcItem)
{
	if ( m_strHoverIcon != _T("") )
	{

		if (!(m_bCheckItem && !GetChecked()))
		{
			CDuiString pStrImage;
			pStrImage.Format(_T("file='%s' dest='%d,%d,%d,%d'"), m_strHoverIcon.GetData(), 
				(ITEM_DEFAULT_ICON_WIDTH - m_szIconSize.cx)/2,
				(m_cxyFixed.cy - m_szIconSize.cy)/2,
				(ITEM_DEFAULT_ICON_WIDTH - m_szIconSize.cx)/2 + m_szIconSize.cx,
				(m_cxyFixed.cy - m_szIconSize.cy)/2 + m_szIconSize.cy);
...
void CMenuElementUI::DrawItemExpland(HDC hDC, const RECT& rcItem)
{
	if (m_bShowExplandIcon)
	{
		...
		strBkImage.Format(_T("file='%s' dest='%d,%d,%d,%d'"), strExplandIcon.GetData(), 
			m_cxyFixed.cx - ITEM_DEFAULT_EXPLAND_ICON_WIDTH + (ITEM_DEFAULT_EXPLAND_ICON_WIDTH - m_szExplandIconSize.cx)/2,
			(m_cxyFixed.cy - m_szExplandIconSize.cy)/2,
			m_cxyFixed.cx - ITEM_DEFAULT_EXPLAND_ICON_WIDTH + (ITEM_DEFAULT_EXPLAND_ICON_WIDTH - m_szExplandIconSize.cx)/2 + m_szExplandIconSize.cx,
			(m_cxyFixed.cy - m_szExplandIconSize.cy)/2 + m_szExplandIconSize.cy);
...
void CMenuElementUI::DoEvent(TEventUI& event)
{
	...
	if( event.Type == UIEVENT_MOUSEENTER )
	{
		...
		m_uMenuElementState |= UISTATE_HOT;
		Invalidate();
		return;
	}
	...
	if( event.Type == UIEVENT_BUTTONUP )
	{
		if( IsEnabled() && IsMouseEnabled()){
			...
			else
			{
				...
				if (CMenuWnd::GetGlobalContextMenuObserver().GetManager() != NULL)
				{
					PostMessage(CMenuWnd::GetGlobalContextMenuObserver().GetManager()->GetPaintWindow(), WM_MENUCLICK, (WPARAM)(&m_sName), (LPARAM)(GetChecked() == TRUE));
				}
				ContextMenuParam param;
				param.hWnd = m_pManager->GetPaintWindow();
				param.type = 1;
...
void CMenuElementUI::SetHoverIcon(LPCTSTR strIcon)
{
	if ( strIcon != _T("") )
		m_strHoverIcon = strIcon;
}
...
void CMenuElementUI::SetExplandIconSize(LONG cx, LONG cy)
{
	m_szExplandIconSize.cx = cx;
	m_szExplandIconSize.cy = cy;
}
...
void CMenuElementUI::SetAttribute(LPCTSTR pstrName, LPCTSTR pstrValue)
{
	...
	else if (_tcscmp(pstrName, _T("itemhovericon")) == 0)
	{
		SetHoverIcon(pstrValue);
	}
	...
	else if( _tcscmp(pstrName, _T("explandiconsize")) == 0 ) {
		LPTSTR pstr = NULL;
		LONG cx = 0, cy = 0;
		cx = _tcstol(pstrValue, &pstr, 10);  ASSERT(pstr);    
		cy = _tcstol(pstr + 1, &pstr, 10);    ASSERT(pstr);   
		SetExplandIconSize(cx, cy);
	}
...
ObserverImpl& GetObserverImplInstance()
{
	return CMenuWnd::GetGlobalContextMenuObserver();
}
