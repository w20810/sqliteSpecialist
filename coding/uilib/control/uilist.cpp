...
namespace UiLib {
			...
			case VK_DOWN:
				{
					...
					{					
						...
						if (!(::GetKeyState(VK_SHIFT) < 0))
							UnSelectAllItems();
						index + 1 > m_pList->GetCount() ? SelectItem(GetCount() - 1, true, false) : SelectItem(index, true, false);
					}
					else if (m_pList->GetCount() > 0)
					{
						SelectItem(0, true);
	...
	int CListUI::GetLastSel() const
	{
		return m_nLastSelect;
	}

	int CListUI::GetCurFirstItem() const
	{
		return 0;
	}

	bool CListUI::SelectItem(int iIndex, bool bTakeFocus, bool bUnSelectOther, bool bSelected)
	{
		...
		if (::GetKeyState(VK_SHIFT) < 0)
		{
			...
			else
			{
				...
				if (m_nRangeBegin == -1 || m_nRangeEnd == -1)
				{
					m_bSelRange = false;
				}
			}
		...
		if( !pListItem->Select(true, bUnSelectOther) ) {		
			return false;
		}

		m_aSelItems.Add((LPVOID)iIndex);
		int pos = m_aSelItemIndex.Find(LPVOID(iIndex));
		if (pos == -1)
			m_aSelItemIndex.Add(LPVOID(iIndex));
	...
	void CListUI::SetSelectedItemFont(int nFont)
	{
		m_ListInfo.nSelectedFont = nFont;
		Invalidate();
	}
	...
	int CListUI::GetSelectedItemFont() const
	{
		return m_ListInfo.nSelectedFont;
	}
	...
	void CListUI::SetAttribute(LPCTSTR pstrName, LPCTSTR pstrValue)
	{
		...
		else if( _tcscmp(pstrName, _T("itemselectedfont")) == 0 ) m_ListInfo.nSelectedFont = _ttoi(pstrValue);
	...
	void CListUI::SelectItemsRange(int nRangeBegin, int nRangeEnd)
	{
		UnSelectAllItems();
		if (nRangeBegin == -1 || nRangeEnd == -1)
			return;
		...
		CControlUI* pControl;
		for (int i = nRangeBegin; i <= nRangeEnd; ++i)
		{
			pControl = GetItemAt(i);
			if(pControl == NULL)
				continue;
	...
	bool CListUI::RemoveSelItemIndex(int iIndex)
	{
		m_aSelItemIndex.Remove(m_aSelItemIndex.Find((LPVOID)(iIndex)));
		m_nLastSelect = iIndex;
		m_bSelRange = false;
		return true;
	}
	...
	void CListUI::UnSelectOtherItems(int iIndex)
	{
		return;
	}
	...
	int CListUI::GetSelItemByIndex(int nIndex) const
	{
		if (nIndex < 0)
			return -1;
	...
	bool CListElementUI::Select(bool bSelect, bool bUnSelectAll, bool bSelected)
	{
	...
	void CListLabelElementUI::DrawItemText(HDC hDC, const RECT& rcItem)
	{
		...
		if( IsSelected() ) {
			if (pInfo->nSelectedFont != -1)
				nFont = pInfo->nSelectedFont;
	...
	bool CListContainerElementUI::Select(bool bSelect, bool bUnSelectAll, bool bSelected)
	{
		if (m_pOwner != NULL && bSelect == true && bUnSelectAll == true)
		{
			// 没有按住shift和ctrl 需要删除当前所有选中
			// 当bSelect == m_bSelected需要在button up时取消除此之外的所有选中 防止拖拽时取消多选
			if (!(::GetKeyState(VK_SHIFT) < 0) && !(::GetKeyState(VK_CONTROL) < 0) && bSelect != m_bSelected)
				m_pOwner->UnSelectAllItems();

			if ((::GetKeyState(VK_CONTROL) < 0) && bSelect == true && bSelect == m_bSelected)
			{
				m_pOwner->RemoveSelItemIndex(m_iIndex);
				bSelect = false;
			}

			if ((::GetKeyState(VK_SHIFT) < 0) && bSelect == true && bSelect == m_bSelected)
			{
				int nLastSelect = m_pOwner->GetLastSel();
				m_pOwner->SelectItemsRange(nLastSelect, m_iIndex + m_pOwner->GetCurFirstItem());
			}
		...
		if( bSelect == m_bSelected ) return true;
		m_bSelected = bSelect;
		if( bSelect && m_pOwner != NULL ) m_pOwner->SelectItem(m_iIndex, false, bUnSelectAll, bSelected);
		Invalidate();
	...
	void CListContainerElementUI::DoEvent(TEventUI& event)
	{
		...
		if (event.Type == UIEVENT_RBUTTONDOWN)
		{
			if (IsEnabled())
			{
				...
				// 没有选中 
					//并且没有按下ctrl或者shift 则选中右键的item 否则弹出右键菜单
				if (bSelected == false && !(::GetKeyState(VK_SHIFT) < 0) && !(::GetKeyState(VK_CONTROL) < 0))
				{
					Select();
					Invalidate();
				}
		...
		if( event.Type == UIEVENT_BUTTONUP ) 
		{
			if (m_pOwner != NULL)
			{
				// 有选中区域 没有按住shift和ctrl 需要删除当前所有选中 （防止当前选中是上一次选中范围内 不可以再次选中）
				if (!(::GetKeyState(VK_SHIFT) < 0) && !(::GetKeyState(VK_CONTROL) < 0))
					m_pOwner->UnSelectOtherItems(m_iIndex);
			}
