...
{
	if (m_re->GetCustomMouse().GetLength() > 0)
	{
		hcur = ::LoadCursorFromFile(CPaintManagerUI::GetResourcePath() + m_re->GetCustomMouse());
	}
    ::SetCursor(hcur);
}
...
CDuiString CRichEditUI::GetCustomMouse()
{
	return m_sCustomMouse;
}

void CRichEditUI::SetCustomMouse(LPCTSTR pStrCustomMouse)
{
	m_sCustomMouse = pStrCustomMouse;
}
	...
	if(/*!m_bFocused && */GetText() == _T(""))
	{
		...
		if( IsEnabled() ) {
			CRenderEngine::DrawText(hDC, m_pManager, rc, sText, mCurTextColor, \
				m_iFont, DT_SINGLELINE);
		}
		else {
			CRenderEngine::DrawText(hDC, m_pManager, rc, sText, m_pManager->GetDefaultDisabledColor(), \
				m_iFont, DT_SINGLELINE);
		}
	}
}
...
void CRichEditUI::SetAttribute(LPCTSTR pstrName, LPCTSTR pstrValue)
{
	...
	else if( _tcscmp(pstrName, _T("custommouse")) == 0 ) SetCustomMouse(pstrValue);
