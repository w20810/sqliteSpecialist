...
{
	class CWebBrowserUI;
	typedef std::map<HWND, CWebBrowserUI*> CIEWndMap;
	...
	public:
		...
		HWND GetServerWnd();
		static LRESULT CALLBACK IEProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam);
	...
	protected:
		...
		HWND m_hServerWnd;
		WNDPROC m_prevProc;
		static CIEWndMap m_browserWndMap;
		...
		void RegIEProc();
		void UnRegIEProc();
		LRESULT IEProcInner(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam);
