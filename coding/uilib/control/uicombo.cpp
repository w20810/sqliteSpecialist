...
LRESULT CComboWnd::HandleMessage(UINT uMsg, WPARAM wParam, LPARAM lParam)
{
    if( uMsg == WM_CREATE ) {
        ...
        if( pControl && _tcscmp(pControl->GetClass(), _T("ScrollBarUI")) != 0 ) PostMessage(WM_KILLFOCUS, (WPARAM)m_pOwner->GetManager()->GetPaintWindow());
    }
        ...
        case VK_ESCAPE:
            ...
            // FALL THROUGH...
        case VK_RETURN:
            PostMessage(WM_KILLFOCUS, (WPARAM)m_pOwner->GetManager()->GetPaintWindow());
            break;
        default:
            TEventUI event;
    ...
    else if( uMsg == WM_KILLFOCUS ) 
	{
        if( m_hWnd != (HWND) wParam )
		{
			...
			m_pm.SendNotify(m_pOwner, DUI_MSGTYPE_CTLWND_KILLFOCUS, wParam, lParam);
		}
    }
...
bool CComboUI::SelectItem(int iIndex, bool bTakeFocus, bool bUnSelectOther, bool bSelected)
{
...
bool CComboUI::SelectItemInit(int iIndex, bool bTakeFocus)
{
	//    if( m_pWindow != NULL ) m_pWindow->Close();  //coed by Redrain 2014.10.30  导致鼠标滚动时自动关闭
	if( iIndex == m_iCurSel ) return true;
	int iOldSel = m_iCurSel;
...
void CComboUI::SelectAllItems()
{
	return;
}

void CComboUI::UnSelectAllItems()
{
	return;
}

void CComboUI::UnSelectOtherItems(int iIndex)
{
	return;
}

void CComboUI::SelectItemsRange(int nRangeBegin, int nRangeEnd)
{
	return;
}

bool CComboUI::RemoveSelItemIndex(int iIndex)
{
	return false;
}

int CComboUI::GetLastSel() const
{
	return -1;
}

int CComboUI::GetCurFirstItem() const
{
	return 0;
}
...
HWND CComboUI::GetComboWnd() const
{
	if (m_pWindow)
...
void CComboUI::SetSelectedFont(int index)
{
	m_ListInfo.nSelectedFont = index;
	Invalidate();
}
