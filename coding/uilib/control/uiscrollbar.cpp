...
namespace UiLib
{
	...
	CScrollBarUI::CScrollBarUI() : m_bHorizontal(false), m_nRange(100), m_nScrollPos(0), m_nLineSize(8), 
		...
		m_uButton2State(0), m_uThumbState(0), m_bShowButton1(true), m_bShowButton2(true), m_nThumbMinHeight(0)
	{
	...
	void CScrollBarUI::SetThumbMinHeight(int minHeight)
	{
		m_nThumbMinHeight = minHeight;
	}
		...
		if( m_bHorizontal ) {
			...
			if( cy > m_cxyFixed.cx ) {
				...
				if( m_nRange > 0 ) {
					...
					if (m_nThumbMinHeight > m_cxyFixed.cx && cyThumb < m_nThumbMinHeight) cyThumb = m_nThumbMinHeight; // 滑块的最小高度大于滚动条的宽度

					m_rcThumb.top = long long(m_nScrollPos) * (cy - cyThumb) / m_nRange + m_rcButton1.bottom;
		...
		if( event.Type == UIEVENT_MOUSEMOVE )
		{
			if( (m_uThumbState & UISTATE_CAPTURED) != 0 ) {
				if( !m_bHorizontal ) {
					...
					if (vRange != 0)
					{
						m_nLastScrollOffset = long long(event.ptMouse.y - ptLastMouse.y) * m_nRange / vRange;
					}
				}
				else {
	...
	void CScrollBarUI::SetAttribute(LPCTSTR pstrName, LPCTSTR pstrValue)
	{
		...
		else if( _tcscmp(pstrName, _T("thumbminwidth")) == 0 ) SetThumbMinHeight(_ttoi(pstrValue));
