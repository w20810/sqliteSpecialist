...
namespace UiLib
{
	...
	COptionUI::COptionUI() : m_bSelected(false), m_dwSelectedTextColor(0), m_dwSelectedBkColor(0), m_nSelectedFont(-1)
	{
	}
	...
	LPCTSTR COptionUI::GetUnSelectedDisableImage()
	{
		return m_sUnSelectedDisableImage;
	}

	void COptionUI::SetUnSelectedDisableImage(LPCTSTR pStrImage)
	{
		m_sUnSelectedDisableImage = pStrImage;
		Invalidate();
	}
	...
	void COptionUI::SetSelectedFont(int nFont)
	{
		m_nSelectedFont = nFont;
	}

	int COptionUI::GetSelectedFont()
	{
		return m_nSelectedFont;
	}
	...
	void COptionUI::SetAttribute(LPCTSTR pstrName, LPCTSTR pstrValue)
	{
		...
		else if( _tcscmp(pstrName, _T("unselecteddisableimage")) == 0 ) SetUnSelectedDisableImage(pstrValue);
	...
	void COptionUI::PaintStatusImage(HDC hDC)
	{
		if( (m_uButtonState & UISTATE_DISABLED) != 0 ) {
			...
			{
				if( !m_sUnSelectedDisableImage.IsEmpty()) {
					if( !DrawImage(hDC, (LPCTSTR)m_sUnSelectedDisableImage) )
						m_sUnSelectedDisableImage.Empty();
					else goto Label_ForeImage;
				}
			}
		}
...
Label_ForeImage:
	...
	void COptionUI::PaintText(HDC hDC)
	{
		if( (m_uButtonState & UISTATE_SELECTED) != 0 )
		{
			...
			if (m_nSelectedFont != -1)
				nFont = m_nSelectedFont;
