...
namespace UiLib
{
	...
	CButtonUI::CButtonUI()
		: m_uButtonState(0), m_dwHotTextColor(0),
		m_dwPushedTextColor(0), m_dwFocusedTextColor(0),
		m_dwHotBkColor(0),m_dwPushedBkColor(0),
		m_iBindTabIndex(-1), m_nHotFont(-1)
	{
		m_uTextStyle = DT_SINGLELINE | DT_VCENTER | DT_CENTER;
		m_wStrMouseStyle = IDC_HAND;
	}
	...
	void CButtonUI::DoEvent(TEventUI& event)
	{
		...
		if( event.Type == UIEVENT_SETCURSOR ) {
			::SetCursor(::LoadCursor(NULL, MAKEINTRESOURCE(m_wStrMouseStyle)));
			return;
		}
	...
	void CButtonUI::SetHotFont(int nHotFont)
	{
		m_nHotFont = nHotFont;
	}
	...
	void CButtonUI::SetAttribute(LPCTSTR pstrName, LPCTSTR pstrValue)
	{
		...
		else if( _tcscmp(pstrName, _T("mousestyle")) == 0 ) SetMouseStyle(pstrValue);
	...
	void CButtonUI::PaintText(HDC hDC)
	{
		...
		{
			if (((m_uButtonState & UISTATE_HOT) != 0) && (GetHotFont() != -1))
			{
				CRenderEngine::DrawText(hDC, m_pManager, rc, m_sText, clrColor, \
				m_nHotFont, m_uTextStyle);
			}
			else
			{
				CRenderEngine::DrawText(hDC, m_pManager, rc, m_sText, clrColor, \
...
Label_ForeImage:
	...
	void CButtonUI::SetMouseStyle(LPCTSTR pStrMouseStyle)
	{
		if(_tcsstr(pStrMouseStyle, _T("arrow")) != NULL) // ¼ýÍ·
		{
			m_wStrMouseStyle = IDC_ARROW;
		}
		else if(_tcsstr(pStrMouseStyle, _T("hand")) != NULL) // ÊÖÐÍ
		{
			m_wStrMouseStyle = IDC_HAND;
		}
	}
