...
namespace wke
{
		...
		,bufHandler_(NULL)
    {
		...
		UChar dir[MAX_PATH + 1] = {0};
		BOOL bOK = GetModuleFileName(NULL, dir, MAX_PATH);
		ASSERT(bOK);
		TCHAR* pBackSlash = wcsrchr(dir, L'\\');
		if (pBackSlash)
			*pBackSlash = L'\0';
	...
	bool CWebView::touchEvent(unsigned int message, int x, int y, unsigned int flags)
	{
		bool handled = false;
		...
		if (message == WM_LBUTTONDOWN || message == WM_MBUTTONDOWN || message == WM_RBUTTONDOWN) {
			Vector<WebCore::PlatformTouchPoint> touchPointsVec;
			WebCore::PlatformTouchPoint touchPoint(0, WebCore::PlatformTouchPoint::TouchPressed, pos, globalPos, 10, 10, 360, 1);
			touchPointsVec.append(touchPoint);
			WebCore::PlatformTouchEvent touchEvent(WebCore::TouchStart, touchPointsVec, ctrl, alt, shift, meta, timestamp);
			handled = mainFrame()->eventHandler()->handleTouchEvent(touchEvent);
		} else if (message == WM_LBUTTONUP || message == WM_MBUTTONUP || message == WM_RBUTTONUP) {
			Vector<WebCore::PlatformTouchPoint> touchPointsVec;
			WebCore::PlatformTouchPoint touchPoint(0, WebCore::PlatformTouchPoint::TouchReleased, pos, globalPos, 10, 10, 360, 1);
			touchPointsVec.append(touchPoint);
			WebCore::PlatformTouchEvent touchEvent(WebCore::TouchEnd, touchPointsVec, ctrl, alt, shift, meta, timestamp);
			handled = mainFrame()->eventHandler()->handleTouchEvent(touchEvent);
		} 
	...
	{
		wke::FrameLoaderClient* frameLoader = NULL;
		frameLoader = static_cast<wke::FrameLoaderClient*>(mainFrame_->loader()->client());
		if (frameLoader != NULL)
			frameLoader->setUserAgent(userAgent);
	}
}
