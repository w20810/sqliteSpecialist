...
namespace wke
{
    ...
    public:
        ...
        {
			wstrUserAgent = "Mozilla/5.0 (Windows NT 6.1) AppleWebKit/535.2 (KHTML, like Gecko) Safari/535.2 wke/1.0";
        }
        ...
        virtual void dispatchDidFinishLoading(WebCore::DocumentLoader*, unsigned long identifier) override
        {
			...
			if (handler == NULL || handler->onLoadingFinish == NULL)
				return;
			...
			handler->onLoadingFinish(handler, (const wkeString)&url.string());
        }
        ...
        virtual void dispatchDidFailProvisionalLoad(const WebCore::ResourceError& error) override
        {
            loadFailed_ = true;
			...
			const wkeClientHandler* handler = webView_->getClientHandler();
			if (handler == NULL || handler->onLoadFail == NULL)
				return;

			handler->onLoadFail(handler, (const wkeString)&error.failingURL(), (const wkeString)&error.localizedDescription(), error.errorCode(), error.isCancellation());
        }

        virtual void dispatchDidFailLoad(const WebCore::ResourceError& error) override
        {
            loadFailed_ = true;
        ...
        virtual void dispatchDidFinishDocumentLoad() override
        {
			...
			const wkeClientHandler* handler = webView_->getClientHandler();
			if (handler == NULL || handler->onDocumentLoadFinish == NULL)
				return;
			...
			handler->onDocumentLoadFinish(handler, (const wkeString)&url.string());
        }
        ...
        virtual void dispatchDidFinishLoad() override
        {
			...
			const wkeClientHandler* handler = webView_->getClientHandler();
			if (handler == NULL || handler->onLoadFinish == NULL)
				return;
			...
			handler->onLoadFinish(handler, (const wkeString)&url.string());
        }
        ...
        virtual void dispatchDecidePolicyForNewWindowAction(WebCore::FramePolicyFunction function, const WebCore::NavigationAction& action, const WebCore::ResourceRequest&, PassRefPtr<WebCore::FormState>, const WTF::String& frameName) override
        {
			if (frame_ == NULL)
				return;

			const wkeClientHandler* handler = webView_->getClientHandler();
			if (handler == NULL || handler->onNewWindow == NULL)
				return;
			...
			const WebCore::ResourceRequest& request = loader->request();
			const WebCore::KURL& srcUrl = request.firstPartyForCookies();
			const WebCore::KURL& destUrl = action.url();

			bool bCancel = false;
			handler->onNewWindow(handler, (const wkeString)&srcUrl.string(), (const wkeString)&destUrl.string(), bCancel);
			if (!bCancel)
				(frame_->loader()->policyChecker()->*function)(WebCore::PolicyUse);
        }

        virtual void dispatchDecidePolicyForNavigationAction(WebCore::FramePolicyFunction function, const WebCore::NavigationAction& action, const WebCore::ResourceRequest&, PassRefPtr<WebCore::FormState>) override
        {
			if (frame_ == NULL)
				return;
		...
		void setUserAgent(const wchar_t* pUserAgent)
		{
			wstrUserAgent = pUserAgent;
		}
