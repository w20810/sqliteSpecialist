...
typedef void (*ON_LOAD_FINISH) (const struct _wkeClientHandler* clientHandler, const wkeString url);
typedef void (*ON_LOAD_FAIL) (const struct _wkeClientHandler* clientHandler, const wkeString failurl, const wkeString localizedDescription, int nErrCode, bool bCancel);
typedef void (*ON_DOCUMENT_LOAD_FINISH) (const struct _wkeClientHandler* clientHandler, const wkeString url);
typedef void (*ON_LOADING_FINISH) (const struct _wkeClientHandler* clientHandler, const wkeString url);
typedef void (*ON_NEW_WINDOW) (const struct _wkeClientHandler* clientHandler, const wkeString srcUrl, const wkeString destUrl, bool& bCancel);
typedef void (*ON_NAVIGATION) (const struct _wkeClientHandler* clientHandler, const wkeString srcUrl, const wkeString destUrl, bool& bCancel);
...
typedef struct _wkeClientHandler {
	...
	ON_DOCUMENT_LOAD_FINISH onDocumentLoadFinish;
	ON_NEW_WINDOW onNewWindow;
	ON_NAVIGATION onNavigation;
	ON_LOADING_FINISH onLoadingFinish;
} wkeClientHandler;
...
namespace wke
{
    ...
    public:
		...
		virtual bool touchEvent(unsigned int message, int x, int y, unsigned int flags) = 0;
...
WKE_API bool wkeTouchEvent(wkeWebView webView, unsigned int message, int x, int y, unsigned int flags);
